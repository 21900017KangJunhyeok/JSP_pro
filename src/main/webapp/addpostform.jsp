<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>

<h1>Add New Post</h1>
<form action="addpost.jsp" method="post" enctype="multipart/form-data">
<table>
<tr><td>분류:</td><td><input type="text" name="category"/></td></tr>
<tr><td>직업:</td><td><input type="text" name="title"/></td></tr>
<tr><td>이름:</td><td><input type="text" name="writer"/></td></tr>
<tr><td>나이:</td><td><input type="text" name="age"/></td></tr>
<tr><td>자기소개:</td><td><textarea cols="50" rows="5" name="content"></textarea></td></tr>
<tr><td>프로필 사진:</td><td><input type="file" name = "photo"></td></tr>
<tr><td><a href="posts.jsp">View All Records</a></td><td align="right"><input type="submit" value="Add Post"/></td></tr>
</table>
</form>

</body>
</html>